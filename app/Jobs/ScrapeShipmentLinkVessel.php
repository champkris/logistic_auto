...
class ScrapeShipmentLinkVessel implements ShouldQueue
{
    ...
    public function __construct(
        ...
        public string $portTerminal = 'B2'
    ) {}

    public function middleware(): array
    {
        return [new RateLimited('shipmentlink-api')];
    }
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            if (!($result['vessel_found'] ?? false)) {
                Log::info("ShipmentLink scrape: vessel not found for {$this->vesselName}/{$this->voyageCode}");
            ...
            Log::info("ShipmentLink scrape completed for {$this->vesselName}", [
            ...
            Log::error("ShipmentLink scrape failed for {$this->vesselName}: " . $e->getMessage());
    ...
    protected function callScraper(): ?array
    {
        ...
        if (!empty($logOutput)) {
            Log::info("ShipmentLink scraper logs for {$this->vesselName}:", ['logs' => $logOutput]);
        }
