...
| **P4.2** | Auto-update berth + UI indicator when berth changes within same terminal | F7 | Migration (add `original_port_terminal`), `CheckAllShipmentsETA.php`, `ShipmentClientController.php`, shipment list Blade | TODO — affects terminals with multiple berths: **LCIT** (B5, C3), **LCB1** (A0, B1), **Hutchison** (C1C2, D1, D2, A2, A3). When ETA check returns a different berth (same terminal), update `port_terminal` on shipment, save original, and show colored indicator in GUI "ท่าเรือ" column (e.g. **C3** <small>was B5</small>) |
| **P4.3** | Berth change detection: update shipment DB + UI when vessel moves berth within same terminal | F7 | `CheckAllShipmentsETA.php`, `ShipmentClientController.php`, shipment list Blade, migration | TODO (next major update) — When ETA check returns the same vessel+voyage+terminal but a **different berth** (e.g. shipment says `C1C2` but scraper returns `D1`), the system should: (1) update `port_terminal` on the shipment to the new berth, (2) store the original berth for audit trail, (3) show a visual indicator in the shipment list UI (e.g. **D1** <small>was C1C2</small>). Currently the berth mismatch is silently ignored. Affected terminals: **Hutchison** (C1C2, D1, D2, A2, A3), **LCIT** (B5, C3), **LCB1** (A0, B1) |
| **P4.4** | Add "(Hutchison Ports)" label to D1, D2, A2, A3 in port dropdown UI | — | Blade port dropdown (shipment form) | TODO (next major update) — Currently only `C1C2` shows "(Hutchison Ports)" in the dropdown. The other Hutchison berths `D1`, `D2`, `A2`, `A3` show as plain text with no terminal label, making it unclear which terminal they belong to. Should display as `D1 (Hutchison Ports)`, `D2 (Hutchison Ports)`, etc. |
| **P4.5** | Voyage fallback mismatch: when no exact voyage match, scraper returns wrong voyage's ETA | F7 | All `*-full-schedule-scraper.js` `findVesselMatch()`, UI shipment list Blade | TODO (next major update) — When a vessel is found but the requested voyage doesn't match any schedule row, all scrapers currently **fall back to the first available voyage** and return its ETA. Example: SAWASDEE SUNRISE / 2602S → LCB1 only has 2603S → scraper returns 2603S ETA → UI shows green "On Track" with wrong date. **Fix:** Instead of silently falling back, return the fallback voyage's data WITH the actual voyage code. UI should display in orange/uncertain color: `📅 17/03 16:00 (2603S)` instead of green. Affected scrapers: **LCB1**, **Hutchison**, **TIPS**, **ESCO**, **LCIT** (all use same `findVesselMatch()` pattern with fallback to `matches[0]`). Requires changes in both JS scrapers (return `fallback_voyage` field) and PHP/Blade (display uncertainty indicator). |
